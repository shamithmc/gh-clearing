...
test('airline sees only confidentiality-safe airport cost index segments', async ({ page }) => {
  ...
  const baggageRow = page.getByRole('row').filter({ hasText: aircraftType }).filter({ hasText: 'Baggage Handling' });
  ...
  await expect(page.getByRole('row').filter({ hasText: aircraftType }).filter({ hasText: 'CLEANING' })).toHaveCount(0);
