...
| **Contract Entry** | Create and edit contracts | Author contracts with 7 pricing formulas (PF-01 to PF-07), edit drafts, submit for approval |
| **Contract Reviewer/Approver** | Contract governance | Approve submitted contracts, request review changes with comments |
| **Invoice Entry** | Invoice creation & calculation | Enter flight turnaround logs, trigger auto-calculation, submit draft invoices |
| **Invoice Approver** | Invoice governance & dispatch | Finalize draft invoices, approve invoices, trigger IS-XML/PDF generation and email dispatch |
...
| **RFP Monitor & Responder** | Procurement & marketplace | Publish/remove station service offerings, view eligible RFPs, submit structured commercial proposals (SOR3) |
...
| **RFP Raiser** | Procurement management | Search marketplace offerings, create and publish RFPs, evaluate supplier rate proposals, award contracts |
...
1. **Check Active Workspace**: Verify whether you are logged into **GH Clearing** or **Airline Clearing**.
...
5. **Check Role Permissions**: Ensure your profile has the necessary role for write actions (e.g., `INVOICE_APPROVER` to send invoices, `DISPUTE_APPROVER` to accept disputes).
